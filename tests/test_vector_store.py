...
class TestQdrantBackend:
    def test_conforms_and_known_answer(self) -> None:
        store = QdrantVectorStore(collection="conformance")
        assert isinstance(store, VectorStoreProtocol)
        store.add(_orthonormal_corpus())
...
def _available_backends() -> list[tuple[str, Callable[[], VectorStoreProtocol]]]:
    ...
    if QDRANT_AVAILABLE:
        items.append(("qdrant", lambda: QdrantVectorStore(collection="contract")))
    return items


@pytest.mark.parametrize("name,build", _available_backends())
class TestContractAcrossBackends:
    ...
    def test_k_below_one_raises(self, name, build) -> None:
        store = build()
        store.add(_orthonormal_corpus())
        with pytest.raises(ValueError):
            store.search(np.array([[1, 0, 0]], dtype="float32"), k=0)

    def test_add_does_not_mutate_caller_array(self, name, build) -> None:
        # Non-unit vectors, so an in-place normalize is detectable.
        store = build()
        corpus = np.array(
    ...
    def test_search_does_not_mutate_query_array(self, name, build) -> None:
        store = build()
        store.add(_orthonormal_corpus())
    ...
    def test_empty_store_returns_zero_width(self, name, build) -> None:
        # The contract is row width min(k, size), so a store built from a 0-row
        # add answers (Nq, 0) on every backend rather than raising.
        store = build()
    ...
    def test_search_before_add_raises_runtimeerror(self, name, build) -> None:
        ...
        with pytest.raises(RuntimeError):
            build().search(np.eye(1, 3, dtype="float32"), k=1)

    def test_one_dim_add_raises_valueerror(self, name, build) -> None:
        # A 1-D add is a caller error, so it must not reach the backend's own
        # index arithmetic and surface as an AxisError.
        with pytest.raises(ValueError, match="2-D"):
            build().add(np.ones(3, dtype="float32"))

    def test_non_finite_embedding_raises_valueerror(self, name, build) -> None:
        # NaN/inf embeddings score as garbage rather than raising, unvalidated.
        store = build()
    ...
    def test_query_dim_mismatch_raises_valueerror(self, name, build) -> None:
        store = build()
